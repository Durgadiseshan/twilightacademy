...
$shape5 = Group_Control_Image_Size::get_attachment_image_html( $data, 'image_size', 'rt_shape5' );
$shape6 = Group_Control_Image_Size::get_attachment_image_html( $data, 'image_size', 'rt_shape6' );
$shape7 = Group_Control_Image_Size::get_attachment_image_html( $data, 'image_size', 'rt_shape7' );
...
<div class="rt-image-banner rt-banner-style-<?php echo esc_attr( $data['style'] );?> <?php if( $data['mouse_animation'] == 'yes' ) { ?>rt-image-parallax<?php } ?>">
	<div class="rt-banner-item <?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $data['delay'] );?>s" data-wow-duration="<?php echo esc_attr( $data['duration'] );?>s">
		<div class="rt-image">
			...
			<ul class="rt-shapes">
				<li class="shape shape1 <?php echo esc_attr( $mouse_animation ); ?>"><?php echo wp_kses_post( $shape1 );?></li>
				...
				<li class="shape shape5 <?php echo esc_attr( $mouse_animation ); ?>"><div data-depth="4.00"><?php echo wp_kses_post( $shape5 );?></div></li>
				<li class="shape shape6 <?php echo esc_attr( $mouse_animation ); ?>"><div data-depth="4.00"><?php echo wp_kses_post( $shape6 );?></div></li>
				<li class="shape shape7 <?php echo esc_attr( $mouse_animation ); ?>"><?php echo wp_kses_post( $shape7 );?></li>
			</ul>
		</div>
	</div>
</div>