...
<div id="<?php echo esc_attr( $accordian_id ) ?>" class="rt-accordion rt-accordion-<?php echo esc_attr( $data['style'] );?>" >
    ...
    foreach ( $data['accordion_repeat'] as $accordion ) {
        ...
        $t = $accordion['title'];
        $selected_icon = ! empty( $accordion['ac_selected_icon']['value'] ) ? $accordion['ac_selected_icon'] : $data['selected_icon'];
        $selected_active_icon = ! empty( $accordion['ac_selected_active_icon']['value'] ) ? $accordion['ac_selected_active_icon'] : $data['selected_active_icon'];

        $uid = strtolower(str_replace(array('%', ':', '\\', '/', '*', '?', '.', ';', ' '), '', $t));
        ?>
            <div class="rt-accordion-item">
                <h2 class="rt-accordion-header <?php if ( $i == 1 ) { ?>active<?php } ?>" id="rtaccordion-<?php echo esc_attr($uid); ?>" >
                    <button class="rt-accordion-button <?php echo esc_attr( $collapse ); ?>" type="button" data-bs-toggle="collapse" data-bs-target="#rtaccordion-collapse-<?php echo esc_attr( $uid ); ?>" aria-expanded="true">
                        <span class="rt-accordion-icon rt-accordion-icon-<?php echo esc_attr( $data['icon_align'] ); ?>" aria-hidden="true">
                            <?php
                            if ( $is_new || $migrated ) { ?>
                                <span class="rt-accordion-icon-closed"><?php Icons_Manager::render_icon( $selected_icon ); ?></span>
                                <span class="rt-accordion-icon-opened"><?php Icons_Manager::render_icon( $selected_active_icon ); ?></span>
                            <?php } else { ?>
