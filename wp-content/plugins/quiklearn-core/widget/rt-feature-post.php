...
Class QuiklearnTheme_Feature_Post extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'rt-feature-post',
			'description' => esc_html__( 'Feature Post Display Widget' , 'quiklearn-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'rt-feature-post', esc_html__( 'Quiklearn : Feature Posts display' , 'quiklearn-core' ), $widget_ops );
	}
	public function widget( $args, $instance ) {
		...
		$title = ( !empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts' , 'quiklearn-core' );
							...
							<a class="post-img-holder img-opacity-hover" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
									if ( has_post_thumbnail() ){
										...
										if ( $show_no_preview_img == 'view' ) { ?>
											<img class="rt-lazy" src="<?php echo esc_url( QUIKLEARN_ASSETS_URL ); ?>img/noimage_400X400.jpg" alt="<?php the_title_attribute(); ?>">
									<?php }
									}
								?></a>
						<?php } ?>
				...
				<div class="media-body">
					<?php if ( $show_date || $show_cat ) { ?>
						<div class="meta-wrap">
							...
							<?php } ?>
							<?php if ( $show_cat ) { ?>
							<div class="entry-cat"><i class="icon-quiklearn-tags"></i>
							<?php
								$i = 1;
