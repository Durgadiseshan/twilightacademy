...
* By : Radius Theme
*/
Class QuiklearnTheme_Product_Box extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'rt-product-box',
			'description' => esc_html__( 'Product Display Widget' , 'quiklearn-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'rt-product-box', esc_html__( 'Quiklearn : Product display Box' , 'quiklearn-core' ), $widget_ops );		
	}
	public function widget( $args, $instance ) {
		...
		$title = ( !empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Product' , 'quiklearn-core' );
				...
				<div class="item-list">
							...
							<div class="post-box-img">
								<a class="post-img" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
									if ( has_post_thumbnail() ){
										the_post_thumbnail( 'quiklearn-size4', ['class' => 'media-object'] );
									} else {
										if ( $show_no_preview_img == 'view' ) { ?>
											<img class="rt-lazy" src="<?php echo esc_url( QUIKLEARN_ASSETS_URL ); ?>img/noimage_375X300.jpg" alt="<?php the_title_attribute(); ?>">
									<?php }
									}
								?></a>
							</div>
					...
					<div class="post-content">
						<div class="post-box-cat">
							<?php if ( $show_cat ) { ?>
								<i class="icon-quiklearn-coading"></i><?php
								$i = 1;
		...
		?>
			...
			<p>
				...
				<select name="<?php echo esc_attr( $this->get_field_name( 'post_display_order' )); ?>">
					...
					<option <?php if ( $post_display_order == 'comment' ) {  echo 'selected'; } ?> value="comment"><?php esc_html_e( 'Most Commented' , 'quiklearn-core' ); ?></option>
				</select>
			</p>
			<p>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' )); ?>"><?php esc_html_e( 'Number of Product to show:', 'quiklearn-core' ); ?></label>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_price' )); ?>"><?php esc_html_e( 'Display product price?' , 'quiklearn-core'); ?></label></p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_cat' )); ?>"><?php esc_html_e( 'Display product category?', 'quiklearn-core' ); ?></label></p>	
