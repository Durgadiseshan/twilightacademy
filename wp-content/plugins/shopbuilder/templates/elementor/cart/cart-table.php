...
$table_horizontal_scroll = ! empty( $controllers['table_horizontal_scroll_on_mobile'] );

?>
<div class="rtsb-cart-table woocommerce<?php echo esc_attr( $table_horizontal_scroll ? ' rtsb-table-horizontal-scroll-on-mobile' : '' ); ?>">
	<?php
	...
	if ( WC()->cart->is_empty() ) {
		...
		<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
			...
			<table class="shop_table cart woocommerce-cart-form__contents <?php echo esc_attr( $table_horizontal_scroll ? '' : 'shop_table_responsive' ); ?>" cellspacing="0">
				<thead>
					<tr>
						<?php
				...
				<tbody>
					...
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
                            ...
                            $show_image_on_mobile = ( $table_horizontal_scroll || $controllers['show_image_on_mobile'] ) ?  ' show-image-on-mobile' : ' hide-image-on-mobile';
                            ?>
