...
class Fns {
	...
	public static function get_prepared_product_id() {
		...
		if ( false === $_post_id || 'publish' !== get_post_status( $_post_id ) ) {
			...
			wp_cache_set( $cache_key, $_post_id, '', 12 * HOUR_IN_SECONDS );
		}
				...
				if ( $raw_option_key === 'active' ) {
					...
					if ( isset( $fields[ $raw_option_key ] ) ) {
						...
						if ( $field['type'] === 'switch' ) {
							...
							$value = stripslashes_deep( $raw_value );
						} else {
	...
	public static function products_category_query( $search_query = null ) {
		...
		if ( ! $results ) {
			...
			if ( $search_query ) {
				...
				$prepared_sql = $wpdb->prepare( $sql, '%' . $wpdb->esc_like( $search_query ) . '%' ); // Escaping and wildcard
			} else {
			...
			wp_cache_set( $cache_key, $results ); // Adjust the expiration time as needed.
		}
