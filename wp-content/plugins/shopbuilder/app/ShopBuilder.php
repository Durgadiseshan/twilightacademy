...
if ( ! class_exists( ShopBuilder::class ) ) {
	/**
	 * Main initialization class.
	 */
	...
	function rtsb() {
		...
		$cached_instance = wp_cache_get('shopbuilder_instance', 'shopbuilder_cache');

		if (false === $cached_instance) {
			$cached_instance = ShopBuilder::instance();
			wp_cache_set('shopbuilder_instance', $cached_instance, 'shopbuilder_cache');
		}
