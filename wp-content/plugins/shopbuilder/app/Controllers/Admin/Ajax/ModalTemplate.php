...
class ModalTemplate {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_rtsb_modal_category_search', [ $this, 'category_search' ] );
	}
	...
	public function response() {
		...
		if ( ! array_intersect( $allowed_roles, $user->roles ) ) {
			wp_die( __( 'You don\'t have permission to perform this action', 'shopbuilder' ) );
		}
		...
		if ( ! is_user_logged_in() && $user->ID !== sanitize_text_field( $_REQUEST['user_id'] ) ) {
			wp_die( __( 'You don\'t have proper authorization to perform this action', 'shopbuilder' ) );
		}
		...
		<form action="<?php echo esc_url( admin_url( 'edit.php?post_type=rtsb_builder' ) ); ?>" autocomplete="off" data-imported-id="<?php echo esc_attr( $_rtsb_import_id ); ?>">
			<div class="rtsb-tb-modal-wrapper">
				...
				<div class="rtsb-product-page-field rtsb-tb-field-wraper" style="display:none;">
					<?php
					$default_items = '';
					...
					?>
					<label for="rtsb_product_page_preview"><?php esc_html_e( 'Select Product For Preview', 'shopbuilder' ); ?></label>
					<select class="rtsb-field" id="rtsb_product_page_preview" name="rtsb_product_page_preview">
						<?php echo Fns::print_html( $default_items, true ); ?>
					</select>
					<p style="margin:0;"> This Product is only used for preview And Elementor edit page. </p>
				</div>

				<div class="rtsb-product-page-field rtsb-page-for-the-products rtsb-tb-field-wraper <?php echo esc_attr( ! rtsb()->has_pro() ? 'pro-disable' : '' ); ?>" style="display:none;">
					<?php
					$default_items_link = '';
					$default_items      = '';
					...
					if ( ! empty( $product_ids ) && is_array( $product_ids ) ) {
						foreach ( $product_ids as $p_id ) {
							...
							$default_items_link .= '<a target="_blank" href="' . get_the_permalink( $p_id ) . '">' . $title . '</a>';
							$default_items      .= '<option selected="selected" value="' . $p_id . '">' . $title . ' - ( ID ' . $p_id . ' )</option>';
						}
					}
					?>
					<label for="rtsb_page_for_the_products "><?php esc_html_e( 'Layout For the Products ', 'shopbuilder' ); ?>
						<?php if ( ! rtsb()->has_pro() ) { ?>
					...
					<p style="margin:0;"> The Products Display in this layout. <span style="display:inline-flex; gap :10px"> <?php echo ! empty( $default_items_link ) ? 'Visit: ' . $default_items_link : ''; ?> </span>
					</p>
				</div>

				<div class="rtsb-categories-page-field rtsb-page-for-the-categories rtsb-tb-field-wraper <?php echo esc_attr( ! rtsb()->has_pro() ? 'pro-disable' : '' ); ?>" style="display:none;">
					<?php
					$default_items_link = '';
					$default_items      = '';
					$categories_name    = apply_filters( 'rtsb/archive/page/builder/selected/categories', [], $post_id );
					if ( ! empty( $categories_name ) && is_array( $categories_name ) ) {
						foreach ( $categories_name as $c_name ) {
							...
							if ( ! is_wp_error( $link ) ) {
								$default_items_link .= '<a target="_blank" href="' . get_term_link( $cat ) . '">' . $title . '</a>';
							}
							$default_items .= '<option selected="selected" value="' . $c_name . '">' . $title . '</option>';
						}
					}
					?>
					<label for="rtsb_page_for_the_products">
						<?php esc_html_e( 'Layout For the Products categories', 'shopbuilder' ); ?>
						<?php if ( ! rtsb()->has_pro() ) { ?>
							<div class="card-label">
								<span class="rtsb-btn-import pro-btn">
									<?php echo esc_html( 'PRO' ); ?>
					...
					<p style="margin:0;"> The Product Categories Display in this layout. <span style="display:inline-flex; gap :10px"> <?php echo ! empty( $default_items_link ) ? 'Visit: ' . $default_items_link : ''; ?> </span>
					</p>
				</div>
				...
				<div class="rtsb-template-setdefaults rtsb-tb-field-wraper">
					...
					if ( $post_id && 'product' === $template_type ) {
						...
						if ( ! empty( $product_ids ) && $set_default ) {
					...
					<span class="rtsb-switch-wrapper"
						  title="<?php esc_html_e( 'Only publish post can set default ', 'shopbuilder' ); ?>">
		...
		<div class="rtsb-template-footer">
			...
			<div class="rtsb-tb-button-wrapper rtsb-tb-edit-button-wrapper">
				<a href="<?php echo esc_url( $url ); ?>" class="btn"> <?php esc_html_e( sprintf( 'Edit with %s', $edit_by ), 'shopbuilder' ); ?> </a>
			</div>
		</div>
		<?php
		$footer = ob_get_clean();
	...
	public function product_search() {
		...
		$_product = wp_cache_get( md5( $query ), 'rtsb-product-query' );
		if ( false === $_product ) {
			$_product = $wpdb->get_results( $query );
			wp_cache_set( md5( $query ), $_product, 'rtsb-product-query' );
		}
		...
		foreach ( $_product as $product ) {
			$product_list['items'][] = [
				'id'   => $product->ID,
				'text' => $product->post_title . ' - ( ID ' . $product->ID . ' )',
			];
		}
