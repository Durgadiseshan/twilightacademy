...
class FilterHooks {
	...
	public static function init_hooks() {
		...
		add_filter( 'woocommerce_add_to_cart_redirect', [ __CLASS__, 'preview_page_product_add_to_cart_redirect' ], 10, 2 );
	 ...
	 * @return false|mixed|string
	 */
	public static function preview_page_product_add_to_cart_redirect( $url, $product ) {
		if ( ! empty( $_GET['demopage'] ) ) {
			$demo_id = $_GET['demopage'];
			if ( BuilderFns::$post_type_tb === get_post_type( $demo_id ) && 'product' === BuilderFns::builder_type( $demo_id ) ) {
				 $url = get_the_permalink( $_GET['demopage'] );
			}
		}
		return $url;
	}
