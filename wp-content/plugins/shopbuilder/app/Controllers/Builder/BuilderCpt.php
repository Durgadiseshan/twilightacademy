...
class BuilderCpt {
	...
	public function add_filter() {
		...
		if ( $typenow != BuilderFns::$post_type_tb ) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended. No need nonce.
	...
	public function template_builder_post_type() {
		...
		$tb_args = [
			...
			'query_var'          => true,
			'publicly_queryable' => apply_filters( 'rtsb/builder/args/publicly_queryable', true ),
	...
	public function custom_columns( $column, $post_id ) {
		...
		$type    = BuilderFns::builder_type( $post_id ) ? BuilderFns::builder_type( $post_id ) : array_key_first( $types );

		switch ( $column ) {
			case 'type':
				$the_type = ! empty( $types[ $type ] ) ? esc_html( ucwords( $types[ $type ] ) ) : '';
				echo apply_filters( 'rtsb/columns/template/types/text', $the_type, $type, $post_id );
				break;
			case 'edit_with':
				...
				break;
			case 'set_default':
				$is_set_default = absint( BuilderFns::builder_page_id_by_type( $type ) );
				...
				<span class="rtsb-switch-wrapper page-type-<?php echo esc_attr( $page_type_for ); ?>" <?php echo ! $is_published ? 'style="pointer-events: none;"' : null; ?> title="<?php esc_html_e( 'Only publish post can set default ', 'shopbuilder' ); ?>">
					<label class="rtsb-switch">
						<?php do_action( 'rtsb/set/default/column/extra/field', $post_id, $type, $is_set_default, $is_published ); ?>
						<div class="rtsb_template_type" data-template_type="<?php echo esc_attr( $type ); ?>" style='display:none;'></div>
