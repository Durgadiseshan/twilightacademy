...
class LayoutFields {
	...
	public static function query( $obj ) {
		...
		if ( rtsb()->has_pro() ) {
			$fields['posts_order_by']['condition'] = [ 'products_filter' => 'recent' ];
		}
	...
	public static function pagination( $obj ) {
		...
		if ( ! rtsb()->has_pro() ) {
			$pagination_description = __( 'Please choose the pagination type.<span class="elementor-pro-notice"><a target="_blank" href="' . esc_url( rtsb()->pro_version_link() ) . '">Upgrade to PRO</a> to unlock Load More and Ajax Pagination.</span>', 'shopbuilder' );
		}
