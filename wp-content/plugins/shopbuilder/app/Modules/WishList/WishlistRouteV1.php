...
class WishlistRouteV1 extends Api {
	...
	public function init() {
		add_action( 'rest_api_init', function () {
			register_rest_route( $this->getNamespace(), '/wishlist/add_to_list', [
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => [ $this, 'add_to_list_callback' ],
				'permission_callback' => [ $this, 'wishlist_permission_callback' ] , //'__return_true'
			] );
			register_rest_route( $this->getNamespace(), '/wishlist/remove_from_list', [
				'methods'             => WP_REST_Server::CREATABLE,
    ...
    public function wishlist_permission_callback( $request ) {
        ...
        if ( $has_permition ) {
            return new WP_Error(
                ...
                __( 'Sorry, you are not allowed to do that!. Only Logged-in user can use this feature' ),
	...
	public function add_to_list_callback( $request ) {
        ...
        if ( $inserted === 0 ) {
            return $this->sendError( esc_html__( 'Product already in wishlist!', 'shopbuilder' ), [
                'action' => 'exist'
            ] );
        }
