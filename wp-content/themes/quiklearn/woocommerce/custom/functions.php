...
class WC_Functions {
	...
	public function __construct() {
		...
		add_action( 'loop_shop_per_page',                  array( $this, 'quiklearn_loop_shop_per_page' ), 20 );
		add_action( 'woocommerce_after_shop_loop',         array( $this, 'quiklearn_products_paginations' ), 10 );
		// Columns
		...
		/* ====== Checkout Page ====== */
		add_filter( 'woocommerce_checkout_fields', array( $this, 'quiklearn_checkout_fields' ) );

		/* ====== Mini Cart ====== */
		add_action( 'quiklearn_woo_cart_icon', array( $this, 'quiklearn_wc_cart_count' ) );
		add_action( 'woocommerce_add_to_cart_fragments', array( $this, 'quiklearn_header_add_to_cart_fragment' ) );
		add_action( 'wp_ajax_quiklearn_product_remove', array( $this, 'quiklearn_product_remove' ) );
        add_action( 'wp_ajax_nopriv_quiklearn_product_remove', array( $this, 'quiklearn_product_remove' ) );
	}
	...
	public function loop_shop_columns(){
		if( isset($_GET["shopview"]) && $_GET["shopview"] == 'list' ) {
			...
			$cols = QuiklearnTheme::$options['products_cols_width'];
		}
		return $cols;
	}

	public function quiklearn_products_paginations(){
		QuiklearnTheme_Helper::pagination();
	}
	...
	public function single_product_layout_hooks() {
		...
		add_action( 'woocommerce_single_product_summary', function(){
			global $product;
			$module_data = [
				'wc_shop_wishlist_icon' => QuiklearnTheme::$options['wc_product_wishlist_icon'],
				'wc_shop_compare_icon' => QuiklearnTheme::$options['wc_product_compare_icon'],
			];?>
			<div class="single-product-yith">
				<?php do_action('rdtheme_after_actions_button_product_page', $product, $module_data); ?>
			</div>
			<?php
		}, 31 );
    ...
    public function quiklearn_loop_shop_per_page( $products ) {
        $shop_posts_per_page = QuiklearnTheme::$options['products_per_page'];
	...
	public static function quiklearn_product_social_share(){
		...
		<div class="post-share-btn">
			<h5 class="item-label"><?php esc_html_e( 'Share:', 'quiklearn' );?></h5>
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'quiklearn' ) : esc_html__( 'Out of Stock', 'quiklearn' );
	}

	public function product_review_form( $comment_form ){
		$commenter = wp_get_current_commenter();
		$comment_form['fields'] = array(
			'author' => '<div class="row"><div class="col-sm-6"><div class="comment-form-author form-group"><input id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" placeholder="' . esc_attr__( 'Name *', 'quiklearn' ) . '" required /></div></div>',
			'email'  => '<div class="comment-form-email col-sm-6"><div class="form-group"><input id="email" class="form-control" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" placeholder="' . esc_attr__( 'Email *', 'quiklearn' ) . '" required /></div></div></div>',
		);

		$comment_form['comment_field'] = '';

		if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
			$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . esc_html__( 'Your Rating', 'quiklearn' ) .'</label>
			<select name="rating" id="rating" required>
			<option value="">' . esc_html__( 'Rate&hellip;', 'quiklearn' ) . '</option>
			<option value="5">' . esc_html__( 'Perfect', 'quiklearn' ) . '</option>
			<option value="4">' . esc_html__( 'Good', 'quiklearn' ) . '</option>
			<option value="3">' . esc_html__( 'Average', 'quiklearn' ) . '</option>
			<option value="2">' . esc_html__( 'Not that bad', 'quiklearn' ) . '</option>
			<option value="1">' . esc_html__( 'Very Poor', 'quiklearn' ) . '</option>
			</select></p>';
		}

		$comment_form['comment_field'] .= '<div class="form-group comment-form-comment"><textarea id="comment" name="comment" class="form-control" placeholder="' . esc_attr__( 'Your Review *', 'quiklearn' ) . '" cols="45" rows="8" required></textarea></div>';

		return $comment_form;
	}

	// WooCommerce Checkout Fields Hook
    public function quiklearn_checkout_fields( $fields ) {
    ...
    public static function QuiklearnWooMiniCart(){
        ob_start();
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            do_action( 'quiklearn_woo_cart_icon' );
        }
     ...
     */
    public function quiklearn_cart_items(){
        $empty_cart = '<li class="cart-item d-flex align-items-center"><h5 class="text-center no-cart-items">'. apply_filters( 'quiklearn_woo_mini_cart_empty', esc_html__('Your cart is empty', 'quiklearn') ) .'</h5></li>';
        ...
        $remove_loader = apply_filters('woo_mini_cart_loader', QuiklearnTheme_Helper::get_img( 'spinner2.gif' ));
                ...
                <li class="cart-item d-flex align-items-center">
	                ...
	                <div class="cart-single-product">
						<div class="media">
										...
										<?php
				                            echo apply_filters(
				                                ...
				                                sprintf(
				                                    ...
				                                    esc_attr__( 'Remove this item', 'quiklearn' ),
				                                    esc_attr( $product_id ),
							...
							<span class="remove-item-overlay text-center"><img src="<?php echo esc_url($remove_loader); ?>" alt="<?php esc_attr_e('Loader..', 'quiklearn') ?>" /></span>
						</div>
					</div>
            ...
            <li class="cart-total">
                <div class="total-price">
                    <span class="f-left"><?php _e( 'Total:', 'quiklearn' ); ?></span>
                    <span class="f-right"><?php echo WC()->cart->get_cart_subtotal(); ?></span>
                </div>
            </li>
            <li class="cart-btn">
                <div class="checkout-link">
                    <a class="button wc-forward" href="<?php echo wc_get_cart_url(); ?>"><?php _e( 'View Cart', 'quiklearn' ); ?></a>
                    <a class="button wc-forward checkout" href="<?php echo wc_get_checkout_url(); ?>"><?php _e( 'Checkout', 'quiklearn' ); ?></a>
                </div>
            </li>
            <?php endif; ?>
        <?php 
    ...
    public function quiklearn_wc_cart_count() {
        ...
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            ...
            <div class="item-icon cart-list-trigger">
	            <a class="cart-contents cart-trigger-icon" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'quiklearn' ); ?>">
				<?php echo quiklearn_cart_shape(); ?><span class="cart-icon-num item-count"><?php echo esc_html( $count > 0  ? absint( $count ) : 0 ); ?></span>
				<span class="cart-amount"><?php echo WC()->cart->get_cart_total(); ?></span>
			</a>
                <div class="cart-wrapper">
                    <ul class="minicart">
		            	<?php echo wp_kses_stripslashes( $this->quiklearn_cart_items() ); ?>
		            </ul>
                </div>
            </div>
            <?php
     ...
     */
    public function quiklearn_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        ...
        ?>
            <div class="header-action-item cart-area mini-cart-items header-shop-cart">
            	<div class="item-icon cart-list-trigger">
	                <a class="cart-contents cart-trigger-icon" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'quiklearn' ); ?>"><?php echo quiklearn_cart_shape(); ?> <?php if ( $count >= 0 ) echo '<span class="cart-icon-num item-count">' . $count . '</span>'; ?>
					<span class="cart-amount"><?php echo WC()->cart->get_cart_total(); ?></span>
				</a>
	                <div class="cart-wrapper">
		                <ul class="minicart">
    ...
    public function quiklearn_wc_cart_ajax() {
        $output = '';
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
        	if(!is_null(WC()->cart)) {
			    $count = WC()->cart->cart_contents_count;
            ...
            ob_start();
            ?>
            <a class="cart-contents" href="<?php echo esc_url( $cart_link ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'quiklearn' ); ?>"><?php echo quiklearn_cart_shape(); ?> <?php if ( $count > 0 ) echo '(' . $count . ')'; ?></a>
            <ul class="minicart">
            <?php
                echo wp_kses_stripslashes( $this->quiklearn_cart_items() );
            ?>
            </ul>
            <?php
            $output = ob_get_clean();
    ...
    public function quiklearn_product_remove() {
        ...
        $return["mini_cart"] = $this->quiklearn_wc_cart_ajax();
    ...
    public function single_meta_actions_button( $product, $module_data ) {
        ...
        $product_id = $product->get_id();
        if( defined( 'RTSB_VERSION' ) && ! empty( $module_data['wc_shop_wishlist_icon'] ) ){ ?>
        	<span class="wishlist">
				<?php do_action( 'rtsb/modules/wishlist/print_button', $product_id );
				echo esc_html("Add To Wishlist", 'quiklearn'); ?>
			</span>
		<?php }
        if( defined( 'RTSB_VERSION' ) && ! empty( $module_data['wc_shop_compare_icon'] ) ){ ?>
        	<span class="compare">
				<?php do_action( 'rtsb/modules/compare/print_button', $product_id );
				echo esc_html("Add To Compare", 'quiklearn'); ?>
			</span>
        <?php }
