...
$id = absint($args['id']);
?>
<div class="<?php em_template_classes('view-container'); ?>" id="em-view-<?php echo esc_attr( $id ); ?>" data-view="event">
	<div class="<?php em_template_classes('event-single'); ?> em-event-<?php echo esc_attr( $EM_Event->event_id); ?> <?php if( $EM_Event->active_status == 0 ) echo 'em-event-cancelled'; ?>" id="em-event-<?php echo esc_attr( $id ); ?>" data-view-id="<?php echo esc_attr( $id ); ?>">
		<?php
		echo esc_attr( $EM_Event->output_single() );
		?>
	</div>
</div>
