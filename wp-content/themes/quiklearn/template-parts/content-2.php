...
$content = wp_trim_words( get_the_excerpt(), QuiklearnTheme::$options['post_content_limit2'], '.' );
...
if (  QuiklearnTheme::$layout == 'right-sidebar' || QuiklearnTheme::$layout == 'left-sidebar' ){
	$post_classes = array( 'col-md-4 col-12 rt-grid-item blog-layout-2 ' . $wow . ' ' . $effect );
	$ul_class = 'side_bar';
} else {
	$post_classes = array( 'col-md-3 col-12 rt-grid-item blog-layout-2 ' . $wow . ' ' . $effect );
	$ul_class = '';
}
...
<div id="post-<?php the_ID(); ?>" <?php post_class( $post_classes ); ?> data-wow-duration="1.5s">
	<div class="blog-box <?php echo esc_attr($img_class); ?> <?php echo esc_attr($preview); ?>">
		...
		<div class="entry-content">
			...
			<?php } ?>			
			<?php if ( QuiklearnTheme::$options['blog_content2'] &&  ! ( QuiklearnTheme::$options['post_content_limit2'] ) == 0 ) { ?>
